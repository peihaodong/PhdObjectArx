...
AcDbObjectId Phd::PhdArxUtility::PostToModelSpace(AcDbEntity* pEnt, AcDbDatabase *pDb /*= acdbCurDwg()*/)
{
	// 检查输入参数的有效性;
...
AcDbObjectId Phd::PhdArxUtility::PostToPaperSpace(AcDbEntity* pEnt, AcDbDatabase *pDb /*= acdbCurDwg()*/)
{
	assert(pEnt);		// 等效于assert (pEnt != NULL);
...
bool Phd::PhdArxUtility::ModifyEntByScale(AcDbEntity* pEnt, double dScale, const AcGePoint3d& ptBase)
{
	AcGeMatrix3d mtx;
...
bool Phd::PhdArxUtility::ModifyEntByScale(const AcArray<AcDbEntity*>& arrpEnt, double dScale,
...
AcArray<AcDbEntity*> Phd::PhdArxUtility::ExplodeEnt(AcDbEntity* pEnt)
{
...
AcArray<AcDbEntity*> Phd::PhdArxUtility::ExplodeEnt(const AcDbObjectId& idEnt)
{
	AcArray<AcDbEntity*> arrExpEnt;
...
bool Phd::PhdArxUtility::MoveEnt(AcDbEntity* pEnt, const AcGePoint3d &ptBase, const AcGePoint3d &ptDest)
{
	assert(pEnt);
...
bool Phd::PhdArxUtility::MoveEnt(const AcArray<AcDbEntity*>& arrEnt, const AcGePoint3d &ptBase, const AcGePoint3d &ptDest)
{
	// 设置变换矩阵的参数
	AcGeMatrix3d xform;
	AcGeVector3d vec(ptDest.x - ptBase.x, ptDest.y - ptBase.y,
...
bool Phd::PhdArxUtility::MoveEnt(const AcDbObjectId& idEnt, const AcGePoint3d &ptBase, const AcGePoint3d &ptDest)
{
...
bool Phd::PhdArxUtility::MoveEnt(const AcDbObjectIdArray& arrid, const AcGePoint3d &ptBase, const AcGePoint3d &ptDest)
{
...
bool Phd::PhdArxUtility::RotateEnt(AcDbEntity *pEnt, const AcGePoint3d &ptBase, double rotation)
{
	assert(pEnt);
...
bool Phd::PhdArxUtility::RotateEnt(const AcArray<AcDbEntity*>& arrpEnt, const AcGePoint3d &ptBase, double rotation)
{
	AcGeMatrix3d xform;
...
AcArray<AcDbCurve*> Phd::PhdArxUtility::OffsetCurve(AcDbCurve* pCurve, double dOffsetDist)
{
...
bool Phd::PhdArxUtility::GetPosAngNegValue(AcDbCurve* pCurve, const AcGePoint3d& ptBase, 
...
AcArray<AcDbCurve*> Phd::PhdArxUtility::OffsetCurve(double dOffset, const AcGePoint3d& pt,
...
AcArray<AcDbCurve*> Phd::PhdArxUtility::SplitCurve(AcDbCurve* pCurve, const AcGePoint3dArray& arrSplitPt)
{
...
AcArray<AcDbCurve*> Phd::PhdArxUtility::SplitCurve(const AcDbObjectId& idCurve,
...
bool Phd::PhdArxUtility::ExtendCurveByDist(AcDbPolyline* pPline, bool isStartPt,
	double dDist)
{
...
bool Phd::PhdArxUtility::ExtendCurve(AcDbCurve* pBorder, AcDbCurve* pExtend,
...
bool Phd::PhdArxUtility::TrimCurve(AcDbCurve* pBorder, AcDbCurve* pTrim,
...
AcArray<AcDbCurve*> Phd::PhdArxUtility::SplitCurve(AcDbCurve* pCurve, const AcGePoint3d& ptSplit)
{
	AcArray<AcDbCurve*> arrSplitCurve;
	AcDbVoidPtrArray spArr;
	AcGePoint3dArray arrptClosed;
...
AcGeCircArc2d Phd::PhdArxUtility::GetGeCircArc(AcDbArc* pArc)
{
...
AcGeLineSeg2d Phd::PhdArxUtility::GetGeLineSeg2d(const AcDbObjectId& idLine)
{
...
AcGeCircArc2d Phd::PhdArxUtility::GetGeCircArc(AcDbCircle* pCircle)
{
...
bool Phd::PhdArxUtility::GetIntersectPoint(const AcGeLine2d& geLine, const AcGeCircArc2d& geCircArc, int& nNum, AcGePoint3dArray& arrptIntersect)
{
	nNum = 0;
	AcGePoint2d pt1, pt2;
...
AcGeVector3d Phd::PhdArxUtility::GetProjectVector(const AcGePoint3d& ptBase, const AcGeVector3d& vctOffset, const AcGeVector3d& vctDirection)
{
...
AcGeVector3d Phd::PhdArxUtility::GetTangentLineVec(const AcGePoint3d& ptOnCurve, const AcDbObjectId& idCurve)
{
	AcDbObjectPointer<AcDbCurve> pCurve(idCurve, AcDb::kForRead);
	if (Acad::eOk != pCurve.openStatus())
...
AcGeVector3dArray Phd::PhdArxUtility::GetTangLineVecs(const AcGePoint3d& pt, const AcDbObjectId& idCurve)
{
	//公差
	double dTol = 0.01;
	AcGeTol tol;
			...
			if (bClosed)
			{
				if (nIndexVertex == 0)
				{
					...
					else
					{
						...
						AcGePoint3d ptCenter = m_apPhdArxConver->ToPt3d(segArc.center());
...
bool Phd::PhdArxUtility::GetTangentLineVecs(AcDbPolyline* pPline, const AcGePoint3d& pt, 
	AcGeVector3dArray& arrvec)
{
	AcGePoint3d ptOnCurve;
	...
	AcGePoint3dArray arrVertex;//顶点集合
	AcArray<double> arrBulge;	//凸度集合
	bool bClosed = m_apPhdArxEntity->IsClosedPline(pPline);
...
AcGeVector3dArray Phd::PhdArxUtility::GetTangLineVecs(const AcGePoint3d& pt, AcDbCurve* pCurve)
{
	//公差
	double dTol = 0.01;
	AcGeTol tol;
...
AcGeVector2d Phd::PhdArxUtility::GetMidVectorOfLines(AcDbLine* pLine1, AcDbLine* pLine2)
{
...
double Phd::PhdArxUtility::GetAngle(const AcGePoint3d& pt, const AcGeVector3d& vec)
{
	AcGePoint3d ptX = pt;
	ptX.x += 1;
...
double Phd::PhdArxUtility::GetCurveLength(AcDbCurve* pCurve)
{
	double dLength = 0;
	double endParam = 0;
...
bool Phd::PhdArxUtility::IsPtOnCurve(const AcGePoint3d& pt, AcDbCurve* pCurve,
	double dTol /*= 0*/)
{
	double dLength = 0;
...
double Phd::PhdArxUtility::GetAngleOfLines(AcDbLine* pLine1, AcDbLine* pLine2)
{
	AcGeLine2d geLine1 = GetGeLine2d(pLine1);
	AcGeLine2d geLine2 = GetGeLine2d(pLine2);
	AcGePoint3d ptInter;
...
AcGePoint3d Phd::PhdArxUtility::GetClosedPtInCurve(AcDbCurve* pCurve, 
...
bool Phd::PhdArxUtility::GetClosedPtInCurve(const AcDbObjectId& idCurve, 
...
AcGePoint3dArray Phd::PhdArxUtility::SortPtArr(const AcGePoint3dArray& PointArr, bool bVertical /*= false*/)
{
...
void Phd::PhdArxUtility::ShowProgressBar(const CString& strName, bool bStart)
{
	if (bStart)
...
void Phd::PhdArxUtility::SetProgressBar(int nCurIndex, int nSum)
{
	int nNum = (nCurIndex + 1) * 100 / nSum;
	acedSetStatusBarProgressMeterPos(nNum);//设置进度条的进度
}

void Phd::PhdArxUtility::SetEntSelected(const AcDbObjectIdArray& arridEnt, bool bSeled /*= true*/, bool bHighlight /*= false*/)
{
...
bool Phd::PhdArxUtility::SetEntToBottom(const AcDbObjectId& id, AcDbDatabase* pDb /*= acdbCurDwg()*/)
{
...
bool Phd::PhdArxUtility::StretchEnt(const AcDbObjectIdArray& arrid, const AcGePoint3d& ptCorner1, const AcGePoint3d& ptCorner2, const AcGePoint3d& ptBase, const AcGePoint3d& ptTarget)
{
...
AcDbObjectIdArray Phd::PhdArxUtility::GetEntIdByPt(const AcGePoint3d& pt, bool bAll /*= false*/)
{
...
AcArray<CString> Phd::PhdArxUtility::GetAllDwgPath(LPCTSTR szFolderPath)
{
...
AcArray<CString> Phd::PhdArxUtility::GetAllDwgName(LPCTSTR szFolderPath)
{
	AcArray<CString> arrDwgPath;
	CFileFind finder;
	CString path;
...
AcDbExtents Phd::PhdArxUtility::GetEntExtents(const AcDbObjectId& idEnt)
{
	AcDbExtents es;
	AcDbEntityPointer pEnt(idEnt,AcDb::kForRead);
	if (Acad::eOk != pEnt.openStatus())
...
AcDbExtents Phd::PhdArxUtility::GetEntExtents(const AcArray<AcDbEntity*>& arrEnt)
{
	AcDbExtents extents;
	for (int i = 0; i < arrEnt.length(); i++)
	{
...
AcDbExtents Phd::PhdArxUtility::GetEntExtents(const AcDbObjectIdArray& arrid)
{
	AcDbExtents extents;
	for (int i = 0; i < arrid.length(); i++)
	{
...
bool Phd::PhdArxUtility::DeleteEnt(const AcDbObjectId& idEnt)
{
	AcDbEntityPointer pEnt(idEnt,AcDb::kForWrite);
...
void Phd::PhdArxUtility::DeleteEnt(AcArray<AcDbEntity*>& arrpEnt)
{
	for (int i = 0; i < arrpEnt.length(); i++)
	{
		if (arrpEnt[i])
...
AcArray<AcDbEntity*> Phd::PhdArxUtility::CopyEnt(const AcDbObjectIdArray& arrid)
{
...
AcDbEntity* Phd::PhdArxUtility::MirrorEnt(AcDbEntity* pEnt, const AcGePoint3d& pt1, const AcGePoint3d& pt2)
{
	AcGeMatrix3d mat;
...
AcArray<AcDbEntity*> Phd::PhdArxUtility::MirrorEnt(const AcArray<AcDbEntity*>& arrp, const AcGePoint3d& pt1, const AcGePoint3d& pt2)
{
...
AcArray<AcDbEntity*> Phd::PhdArxUtility::MirrorEnt(const AcDbObjectIdArray& arrid, const AcGePoint3d& pt1, const AcGePoint3d& pt2)
{
	AcArray<AcDbEntity*> arrpEnt;
	AcGeMatrix3d mat;
	mat.setToMirroring(AcGeLine3d(pt1, pt2));
...
AcArray<AcDbEntity*> Phd::PhdArxUtility::CopyEnt(const AcArray<AcDbEntity*>& arrp)
{
	AcArray<AcDbEntity*> arrEnt;
...
AcDbEntity* Phd::PhdArxUtility::CopyEnt(const AcDbObjectId& idEnt)
{
	AcDbEntityPointer pEnt(idEnt,AcDb::kForRead);
	if (Acad::eOk != pEnt.openStatus())
		return NULL;
...
int Phd::PhdArxUtility::GetLineType(const AcDbObjectId& idLine, double& dPosition)
{
	AcDbObjectPointer<AcDbLine> pLine(idLine,AcDb::kForRead);
	if (Acad::eOk != pLine.openStatus())
		return 0;
	AcGePoint3d ptStart = pLine->startPoint();
	AcGePoint3d ptEnd = pLine->endPoint();
	if (m_apPhdArxInline->IsEqual(ptStart.y,ptEnd.y) && !m_apPhdArxInline->IsEqual(ptStart.x,ptEnd.x))
	{
		dPosition = ptStart.y;
		return 1;
	}
	else if (m_apPhdArxInline->IsEqual(ptStart.x, ptEnd.x) && !m_apPhdArxInline->IsEqual(ptStart.y, ptEnd.y))
	{
		dPosition = ptStart.x;
		return 2;
	}
...
int Phd::PhdArxUtility::GetLineType(AcDbLine* pLine, double& dPosition)
{
	AcGePoint3d ptStart = pLine->startPoint();
	AcGePoint3d ptEnd = pLine->endPoint();
	if (m_apPhdArxInline->IsEqual(ptStart.y, ptEnd.y) && !m_apPhdArxInline->IsEqual(ptStart.x, ptEnd.x))
	...
	else if (Phd::PhdArxUtility::PtIsEqual(ptStart,ptEnd,1))//如果两个点相等，则判断这条直线是水平
	{
		dPosition = ptStart.y;
		return 1;
	}
...
bool Phd::PhdArxUtility::GetExtentsPt(const AcDbObjectId& idEnt, AcGePoint3d& ptMax, AcGePoint3d& ptMin)
{
	AcDbEntityPointer pEnt(idEnt,AcDb::kForRead);
	if (Acad::eOk != pEnt.openStatus())
		return false;
...
bool Phd::PhdArxUtility::GetExtentsPt(const AcDbObjectIdArray& arrid, AcGePoint3d& ptMax, AcGePoint3d& ptMin)
{
	AcDbExtents extents;
	for (int i = 0; i < arrid.length(); i++)
	{
...
bool Phd::PhdArxUtility::GetExtentsPt(AcDbEntity* pEnt, AcGePoint3d& ptMax, AcGePoint3d& ptMin)
{
	AcDbExtents extents;
...
bool Phd::PhdArxUtility::PtIsEqual(const AcGePoint3d& pt1, const AcGePoint3d& pt2, int nDotNum)
{
	double dX1 = m_apPhdArxConver->UpdateOnDotNum(pt1.x, nDotNum);
	double dY1 = m_apPhdArxConver->UpdateOnDotNum(pt1.y, nDotNum);
	double dZ1 = m_apPhdArxConver->UpdateOnDotNum(pt1.z, nDotNum);
	double dX2 = m_apPhdArxConver->UpdateOnDotNum(pt2.x, nDotNum);
	double dY2 = m_apPhdArxConver->UpdateOnDotNum(pt2.y, nDotNum);
	double dZ2 = m_apPhdArxConver->UpdateOnDotNum(pt2.z, nDotNum);
...
bool Phd::PhdArxUtility::LineIsCoincide(AcDbLine* pLine1, AcDbLine* pLine2)
{
...
bool Phd::PhdArxUtility::LineIsCompleteContain(AcDbLine* pLongLine, AcDbLine* pShortLine)
{
	double dLongLength = GetCurveLength(pLongLine);
	double dShortLength = GetCurveLength(pShortLine);
	if (dShortLength > dLongLength || m_apPhdArxInline->IsEqual(dShortLength,dLongLength,1))
		return false;
...
AcGePoint3dArray Phd::PhdArxUtility::RemoveSamePt(const AcGePoint3dArray& arrpt)
{
	...
	for (int i = 0; i < arrpt.length(); i++)
	{
		...
		{
			if (Phd::PhdArxUtility::PtIsEqual(pt, arrptSingle[j], 0))
			{
				bContains = true;
				break;
			}
...
CString Phd::PhdArxUtility::GetEntLayerName(AcDbEntity* pEnt)
{
...
bool Phd::PhdArxUtility::IsAccordId(const AcDbObjectId& idEnt, const AcArray<AcRxClass*>& arrType)
{
	AcDbEntityPointer pEnt(idEnt, AcDb::kForRead);
	if (Acad::eOk != pEnt.openStatus())
		return false;
...
bool Phd::PhdArxUtility::IsContainsPt(const AcGePoint3dArray& arrPt, const AcGePoint3d& pt, double dTol /*= 0.01*/)
{
	//公差
	AcGeTol tol;
	tol.setEqualPoint(dTol);
...
bool Phd::PhdArxUtility::ReversePolylinePt(AcDbPolyline* pPline)
{
	bool bClosed = pPline->isClosed();
	...
	if (bClosed)
	{
		...
		{
			pPline->addVertexAt(i, m_apPhdArxConver->ToPt2d(arrPt[nCount - i]),
				arrBulge[nCount - i - 1]);
		}
	}
	else
	{
		...
		for (int i = nCount - 1; i > 0; i--)
			pPline->removeVertexAt(i);
		//设置第一点
		pPline->setPointAt(0, m_apPhdArxConver->ToPt2d(arrPt[nCount - 1]));
		pPline->setBulgeAt(0, arrBulge[nCount - 2]);
		//增加点
		for (int i = 1; i < nCount - 1; i++)
		{
			pPline->addVertexAt(i, m_apPhdArxConver->ToPt2d(arrPt[nCount - i - 1]),
				arrBulge[nCount - i - 2]);
		}
		//设置最后一点
		pPline->addVertexAt(nCount - 1, m_apPhdArxConver->ToPt2d(arrPt[0]),
			arrBulge[nCount - 1]);
	}
	return true;
}

AcDbPolyline* Phd::PhdArxUtility::ReversePlinePt(AcDbPolyline* pPline)
{
	bool bClosed = pPline->isClosed();
	AcGePoint3dArray arrPt;	//点集合
	AcArray<double> arrBulge;	//凸度集合
	...
	if (bClosed)
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			if (i == 0)
			{
				pPlineNew->addVertexAt(i, m_apPhdArxConver->ToPt2d(arrPt[0]),
					arrBulge[nCount - 1]);
			}
			else
			{
				pPlineNew->addVertexAt(i, m_apPhdArxConver->ToPt2d(arrPt[nCount - i]),
					arrBulge[nCount - i - 1]);
			}
		}
	...
	else
	{
		...
		for (int i = 0; i < nCount - 1; i++)
		{
			if (i == 0)
			{
				pPlineNew->addVertexAt(i, m_apPhdArxConver->ToPt2d(arrPt[nCount - 1]),
					arrBulge[nCount - 2]);
			}
			else
			{
				pPlineNew->addVertexAt(i, m_apPhdArxConver->ToPt2d(arrPt[nCount - i - 1]),
					arrBulge[nCount - i - 2]);
			}
		}
		//设置最后一点
		pPlineNew->addVertexAt(nCount - 1, m_apPhdArxConver->ToPt2d(arrPt[0]),
			arrBulge[nCount - 1]);
		return pPlineNew;
	}
}

AcDbPolyline* Phd::PhdArxUtility::Pline2dToPline(AcDb2dPolyline* p2dPline)
{
	if (!p2dPline)
		return NULL;
...
bool Phd::PhdArxUtility::IsMirrorArc(AcDbArc* pArc)
{
...
bool Phd::PhdArxUtility::FilletCurve(double dRadius, AcDbLine* pLine1, const AcGePoint3d& pt1, AcDbArc* pArc2, const AcGePoint3d& pt2, AcDbArc*& pArcFillet)
{
	//偏移线段
	...
	//求切点
	AcDbCircle* pCircleTemp = m_apPhdArxEntity->CreateCircle(ptCenter, dRadius);
	...
	//创建圆角圆弧
	AcDbArc* pArcFillet1 = m_apPhdArxEntity->CreateArcByPt2(ptCenter, arrptInter1[0], arrptInter2[0]);
	AcDbArc* pArcFillet2 = m_apPhdArxEntity->CreateArcByPt2(ptCenter, arrptInter2[0], arrptInter1[0]);
	AcGePoint3d ptMid1 = m_apPhdArxEntity->GetArcMidPt(pArcFillet1);
	AcGePoint3d ptMid2 = m_apPhdArxEntity->GetArcMidPt(pArcFillet2);
	...
	m_apPhdArxEntity->ModifyArcPt(pArc2, ptInter, arrptInter2[0]);

	return true;
}

void Phd::PhdArxUtility::ModifyLinePtToInterPt(AcDbLine* pLine, const AcGePoint3d& ptOnLine, const AcGePoint3d& ptInter)
{
...
void Phd::PhdArxUtility::ModifyArcPtToInterPt(AcDbArc* pArc, const AcGePoint3d& ptOnArc, const AcGePoint3d& ptInter)
{
	double dStartParam = 0, dEndParam = 0, dOnArcParam = 0, dInterParam = 0;
...
void Phd::PhdArxUtility::ModifyLinePt(AcDbLine* pLine, const AcGePoint3d& ptFrom, const AcGePoint3d& ptTo)
{
	AcGePoint3d ptStart, ptEnd;
...
CString Phd::PhdArxUtility::GetRcStr(int nID)
{
...
AcGeVector3dArray Phd::PhdArxUtility::GetPerpVec(const AcGeVector3d& vecDirec)
{
	AcGeVector3dArray arrvec;
...
bool Phd::PhdArxUtility::IsAntiClockWise(const AcGePoint3dArray& arrpt)
{
	double area = 0;
...
double Phd::PhdArxUtility::GetPtArrayArea(const AcGePoint3dArray& arrpt)
{
...
int Phd::PhdArxUtility::GetPlineSegIndexByPt(AcDbPolyline* pPline, const AcGePoint3d& pt,
	bool& bIsArc)
{
...
int Phd::PhdArxUtility::JudgeVectorLeftOrRight(const AcGeVector3d& vec1,
...
bool Phd::PhdArxUtility::PtIsInClosePline(const AcGePoint3d& pt,
	AcDbPolyline* pPline)
{
	//创建面域
	AcArray<AcDbRegion*> arrRegion;
	if (!m_apPhdArxEntity->CreateRegion(pPline, arrRegion))
		return false;
	//判断点是否在面域上
	AcGePlane plane;
	bool bFlag = false;
...
bool Phd::PhdArxUtility::IsColinear(const AcGePoint3d& pt1, const AcGePoint3d& pt2, const AcGePoint3d& pt3)
{
	AcGeLineSeg2d line1(m_apPhdArxConver->ToPt2d(pt1),m_apPhdArxConver->ToPt2d(pt2));
	AcGeLineSeg2d line2(m_apPhdArxConver->ToPt2d(pt2), m_apPhdArxConver->ToPt2d(pt3));
	bool bRt = line1.isColinearTo(line2);
	return bRt;
}

bool Phd::PhdArxUtility::PlineAddPts(AcDbPolyline* pPline, const AcGePoint3dArray& arrpt)
{
	...
	for (int i = 0; i < arrpt.length(); i++)
	{
		...
		//创建新的多段线
		AcDbPolyline* pNewPline = m_apPhdArxEntity->CreatePolyline(arrptTemp, arrdTemp);
		DEL(pTempPline);
...
bool Phd::PhdArxUtility::FilletCurve(double dRadius, AcDbArc* pArc1, const AcGePoint3d& pt1, AcDbArc* pArc2, const AcGePoint3d& pt2, AcDbArc*& pArcFillet)
{
	//偏移线段
	...
	//第二次修改圆弧（交点处修改圆弧）
	m_apPhdArxEntity->ModifyArcPt(pArc1, ptInter, arrptInter1[0]);
	m_apPhdArxEntity->ModifyArcPt(pArc2, ptInter, arrptInter2[0]);

	return true;
}

Phd::PhdArxUtility::PhdArxUtility()
...
CString Phd::PhdArxUtility::GetCurZrxFilePath()
{
...
CString Phd::PhdArxUtility::GetCurZrxFolderPath()
{
	TCHAR szPath[_MAX_PATH];
	::GetModuleFileName(_hdllInstance, szPath, _MAX_PATH);
...
bool Phd::PhdArxUtility::LoadCadMenu(const CString& strMenuFileName, const CString& strMenuGroupName)
{
...
bool Phd::PhdArxUtility::UnloadCadMenu(const CString& strMenuGroupName)
{
	CString strCmdLoad;
...
bool Phd::PhdArxUtility::LoadZrxFile(LPCTSTR szFilePath)
{
	bool bRet = false;
...
bool Phd::PhdArxUtility::AddSearchPath(LPCTSTR szSearchDir)
{
	CFileStatus st;
...
bool Phd::PhdArxUtility::GetIntersectPoint(const AcGeLineSeg2d& geLine, const AcGeCircArc2d& geCircArc, int& nNum, AcGePoint3dArray& arrptIntersect)
{
	nNum = 0;
	AcGePoint2d pt1, pt2;
	if (!geCircArc.intersectWith(geLine, nNum, pt1, pt2))
...
bool Phd::PhdArxUtility::GetIntersectPoint(const AcGeLineSeg2d& geLine1, const AcGeLineSeg2d& geLine2, AcGePoint3d& ptIntersect)
{
	AcGePoint2d pt2d;
...
bool Phd::PhdArxUtility::GetIntersectPoint(const AcGeLineSeg2d& geLine1, const AcGeLine2d& geLine2, AcGePoint3d& ptIntersect)
{
	AcGePoint2d pt2d;
	bool bRet = geLine1.intersectWith(geLine2, pt2d);
	if (!bRet)
...
bool Phd::PhdArxUtility::GetIntersectPoint(const AcGeLine2d& geLine1, const AcGeLine2d& geLine2, 
...
AcArray<AcDbEntity*> Phd::PhdArxUtility::AnnularMatrix(const AcDbObjectIdArray& arrid, const AcGePoint3d& ptBase, int nCurNum, int nSum)
{
	...
	{
		AcArray<AcDbEntity*> arrpCopy = Phd::PhdArxUtility::CopyEnt(arrid);
		Phd::PhdArxUtility::RotateEnt(arrpCopy, ptBase, m_apPhdArxConver->ToRadian(arrAngle[i]));
...
AcArray<AcDbEntity*> Phd::PhdArxUtility::AnnularMatrix(const AcArray<AcDbEntity*>& arrp,
	const AcGePoint3d& ptBase, int nYxcs, int nSum)
{
	...
	for (int i = 0; i < arrAngle.length(); i++)
	{
		AcArray<AcDbEntity*> arrpCopy = Phd::PhdArxUtility::CopyEnt(arrp);
		Phd::PhdArxUtility::RotateEnt(arrpCopy, ptBase, m_apPhdArxConver->ToRadian(arrAngle[i]));
		arrpMatrix.append(arrpCopy);
	}
...
double Phd::PhdArxUtility::GetArcBulge(AcDbArc* pArc)
{
	double dStartAngle = pArc->startAngle();
	double dEndAngle = pArc->endAngle();
...
double Phd::PhdArxUtility::GetArcBulge(const AcGeCircArc2d& geArc)
{
...
double Phd::PhdArxUtility::GetArcBulge(AcDbCurve* pCurve)
{
	double dBulge = 0.0;
...
double Phd::PhdArxUtility::GetArcBulge(const AcDbObjectId& idArc)
{
...
AcGeLine2d Phd::PhdArxUtility::GetGeLine2d(AcDbLine* pLine)
{
	AcGePoint3d ptStart = pLine->startPoint();
	AcGePoint3d ptEnd = pLine->endPoint();
	AcGeLine2d geLine;
...
AcGeLineSeg2d Phd::PhdArxUtility::GetGeLineSeg2d(AcDbLine* pLine)
{
	AcGePoint3d ptStart = pLine->startPoint();
	AcGePoint3d ptEnd = pLine->endPoint();
	AcGeLineSeg2d geLine;
...
AcGeCircArc2d Phd::PhdArxUtility::GetGeCircArc2d(AcDbPolyline* pPolyline, unsigned int unIndex)
{
	AcGeCircArc2d geCircArc;
...
AcGeLineSeg2d Phd::PhdArxUtility::GetGeLineSeg2d(AcDbPolyline* pPolyline, unsigned int unIndex)
{
	AcGeLineSeg2d geLine;
...
double Phd::PhdArxUtility::GetArcBulge(double dAngleStart, double dAngleEnd)
{
...
bool Phd::PhdArxUtility::GetIntersectPoint(const AcGeCircArc2d& geCirArc1, const AcGeCircArc2d& geCirArc2, int& nNum, AcGePoint3dArray& arrptIntersect)
{
	AcGePoint2d pt1, pt2;
	nNum = 0;
...
bool Phd::PhdArxUtility::ModifyEntByScale(const AcDbObjectId& idEnt, double dScale, const AcGePoint3d& ptBase)
{
	AcDbEntityPointer pEnt(idEnt, AcDb::kForWrite);
	if (pEnt.openStatus() != Acad::eOk)
		return false;
