...
Phd::PhdArxView::PhdArxView()
	:m_apPhdArxConver(std::make_shared<Phd::PhdArxConver>())
{

}

void Phd::PhdArxView::GetCurrentView(AcDbViewTableRecord &view)
{
	struct resbuf rb;
...
void Phd::PhdArxView::SetViewExtent(const AcGePoint3d &ptMin, const AcGePoint3d &ptMax, double scale /*= 1.0*/)
{
	AcDbViewTableRecord view;
	GetCurrentView(view);

	// 将参数的两个点从世界坐标系转换到显示坐标系
	AcGePoint2d ptMinDcs = m_apPhdArxConver->UcsToDcsPt(m_apPhdArxConver->WcsToUcsPt(ptMin));
	AcGePoint2d ptMaxDcs = m_apPhdArxConver->UcsToDcsPt(m_apPhdArxConver->WcsToUcsPt(ptMax));
...
double Phd::PhdArxView::GetCurViewHeight()
{
	AcDbViewTableRecord view;
	GetCurrentView(view);
...
void Phd::PhdArxView::GetCurViewData(double& dHeight, double& dWidth, AcGePoint2d& ptCenter)
{
	AcDbViewTableRecord view;
	GetCurrentView(view);
...
void Phd::PhdArxView::SetCurViewData(double dHeight, double dWidth, const AcGePoint2d& ptCenter)
{
	AcDbViewTableRecord view;
	GetCurrentView(view);
