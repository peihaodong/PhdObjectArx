...
AcDbWipeout* Phd::PhdArxEntity::CreateWipeout(const AcGePoint3d& ptMin, const AcGePoint3d& ptMax)
{
...
AcDbPolyline* Phd::PhdArxEntity::CreatePolyline(const AcGePoint3dArray& ptArr)
{
	if (ptArr.length() < 2)
		return NULL;
...
AcDbPolyline* Phd::PhdArxEntity::CreatePolyline(const AcGePoint3dArray& arrpt,
	const AcArray<double>& arrdBulge)
{
	//创建多段线
	AcDbPolyline* pPoly = new AcDbPolyline();
	for (int i = 0; i < arrpt.length(); i++)
		pPoly->addVertexAt(i, m_apPhdArxConver->ToPt2d(arrpt[i]),arrdBulge[i]);
	return pPoly;
}

bool Phd::PhdArxEntity::IsClosedPline(AcDbPolyline* pPline)
{
...
AcDbPolyline* Phd::PhdArxEntity::CreatePolyline(const AcGePoint3d& ptMin, const AcGePoint3d& ptMax)
{
	AcDbPolyline* pPoly = new AcDbPolyline();
...
bool Phd::PhdArxEntity::CreateRegion(const AcArray<AcDbEntity*>& arrpEnt, AcArray<AcDbRegion*>& arrpRegion)
{
...
bool Phd::PhdArxEntity::CreateRegion(AcDbEntity* pEnt, AcArray<AcDbRegion*>& arrpRegion)
{
	AcDbVoidPtrArray curves;
...
AcDbArc* Phd::PhdArxEntity::CreateArc(const AcGePoint3d& ptStart, const AcGePoint3d& ptOnArc, const AcGePoint3d& ptEnd)
{
	// 使用几何类获得圆心、半径;
...
// AcDbArc* Phd::PhdArxEntity::CreateArcByPt(const AcGePoint3d& ptCenter, const AcGePoint3d& pt1, const AcGePoint3d& pt2)
// {
// 	// 计算半径;
...
// 	double startAngle = vecStart.angle();
// 	double endAngle = vecEnd.angle();
// 	double dBulge = m_apPhdArxUtility->GetArcBulge(startAngle, endAngle);
// 	if (0 > dBulge)
// 	{
...
// 		dBulge = m_apPhdArxUtility->GetArcBulge(startAngle, endAngle);
// 		if (0 > dBulge)
// 			return NULL;
// 	}
...
AcDbArc* Phd::PhdArxEntity::CreateArcByPt2(const AcGePoint3d& ptCenter, const AcGePoint3d& ptStart, 
...
// AcDbArc* Phd::PhdArxEntity::CreateArcBy3Pt(const AcGePoint3d& pt1,
// 	const AcGePoint3d& ptOnArc, const AcGePoint3d& pt2)
// {
// 	AcGeCircArc2d geArc(m_apPhdArxConver->ToPt2d(pt1),
// 		m_apPhdArxConver->ToPt2d(ptOnArc), m_apPhdArxConver->ToPt2d(pt2));
// 	AcGePoint2d ptCenter = geArc.center();
// 	double dRadius = geArc.radius();
// 	AcDbCircle* pCircle = CreateCircle(m_apPhdArxConver->ToPt3d(ptCenter),dRadius);
// 	std::shared_ptr<AcDbCircle> apCircle(pCircle);
// 	AcGePoint3dArray arrptSplit;
// 	arrptSplit.append(pt1);
// 	arrptSplit.append(pt2);
// 	AcArray<AcDbCurve*> arrpSplit = m_apPhdArxUtility->SplitCurve(pCircle,arrptSplit);
...
// 	}
// 	//判断该点是否在圆弧上
// 	if (m_apPhdArxUtility->IsPtOnCurve(ptOnArc, pArcSplit1))
// 	{
// 		DEL(pArcSplit2);
// 		return pArcSplit1;
// 	}
...
AcDbText* Phd::PhdArxEntity::CreateTextForCenter(double dHeight, const AcGePoint3d& pt, LPCTSTR szText)
{
...
AcDbText* Phd::PhdArxEntity::CreateTextForCenter(const AcGePoint3d& pt, LPCTSTR szText, const AcDbObjectId& idProEnt)
{
...
AcDbText* Phd::PhdArxEntity::CreateTextForLeftTop(double dHeight, const AcGePoint3d& pt, LPCTSTR szText)
{
	AcDbText* pText = new AcDbText(pt, szText, AcDbObjectId::kNull, dHeight);
...
AcDbText* Phd::PhdArxEntity::CreateTextForLeft(double dHeight, const AcGePoint3d& pt, LPCTSTR szText)
{
	AcDbText* pText = new AcDbText(pt, szText, AcDbObjectId::kNull, dHeight);
 	pText->setAlignmentPoint(pt);
	return pText;
...
AcDbText* Phd::PhdArxEntity::CreateTextForLeft(const AcGePoint3d& pt, LPCTSTR szText, const AcDbObjectId& idProEnt)
{
	AcDbObjectPointer<AcDbText> pText(idProEnt,AcDb::kForRead);
	if (Acad::eOk != pText.openStatus())
		return NULL;
...
AcDbText* Phd::PhdArxEntity::CreateTextForLeft(double dHeight, const AcGePoint3d& pt, LPCTSTR szText, AcDbEntity* pProEnt)
{
	AcDbText* pText = new AcDbText(pt, szText, AcDbObjectId::kNull, dHeight);
	pText->setAlignmentPoint(pt);
	if (pProEnt)
...
AcDbTable* Phd::PhdArxEntity::CreateTable(const AcGePoint3d& ptInster)
{
...
AcDbLine* Phd::PhdArxEntity::CreateLine(const AcGePoint3d& ptStart, const AcGePoint3d& ptEnd)
{
	AcDbLine* pLine = new AcDbLine(ptStart,ptEnd);
	return pLine;
}

AcDbLine* Phd::PhdArxEntity::CreateLine(const AcGePoint3d& ptStart, const AcGePoint3d& ptEnd, const AcDbObjectId& idProEnt)
{
	AcDbEntityPointer pEnt(idProEnt,AcDb::kForRead);
	if (Acad::eOk != pEnt.openStatus())
		return NULL;
...
AcDbLine* Phd::PhdArxEntity::CreateLine(const AcGePoint3d& ptStart, const AcGePoint3d& ptEnd, AcDbEntity* pProEnt)
{
	AcDbLine* pLine = new AcDbLine(ptStart, ptEnd);
	if (pProEnt)
...
AcDbLeader* Phd::PhdArxEntity::CreateLeader(const AcGePoint3dArray& arrpt, 
...
AcDbPoint* Phd::PhdArxEntity::CreatePoint(const AcGePoint3d& pt)
{
	AcDbPoint* pPt = new AcDbPoint(pt);
	return pPt;
}

// AcDbArc* Phd::PhdArxEntity::CreateArc(const AcGePoint3d& ptStart, const AcGePoint3d& ptEnd, double dRadius)
// {
...
// 	int nNum = 0;
// 	bool bRet = m_apPhdArxUtility->GetIntersectPoint(geCircle1, geCircle2, nNum, arrptInter);
// 	if (!bRet)
// 		return NULL;
...
AcDbWipeout* Phd::PhdArxEntity::CreateWipeout(const AcGePoint3dArray& ptArr)
{
	if (ptArr.length() < 3)
		return NULL;
...
AcGePoint3d Phd::PhdArxEntity::GetArcMidPt(AcDbArc* pArc)
{
...
bool Phd::PhdArxEntity::ModifyArcPt(AcDbArc* pArc, const AcGePoint3d& ptFrom,
...
AcDbCircle* Phd::PhdArxEntity::CreateCircle(const AcGePoint3d& ptCenter, double dRadius)
{
