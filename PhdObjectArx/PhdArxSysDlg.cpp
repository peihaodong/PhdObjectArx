...
void Phd::PhdArxSysDlg::AlertBox(LPCTSTR msg)
{
	AfxMessageBox(msg, MB_OK | MB_ICONEXCLAMATION);
}

void Phd::PhdArxSysDlg::StopAlertBox(LPCTSTR msg)
{
	AfxMessageBox(msg, MB_OK | MB_ICONSTOP);
}

void Phd::PhdArxSysDlg::PromptBox(LPCTSTR msg)
{
	AfxMessageBox(msg, MB_OK | MB_ICONINFORMATION);
}

UINT Phd::PhdArxSysDlg::QuestionBox(LPCTSTR msg, UINT defButton /*= IDYES*/)
{
	UINT theDefault = (defButton == IDYES) ? MB_DEFBUTTON1 : MB_DEFBUTTON2;
	return(AfxMessageBox(msg, MB_YESNO | MB_ICONQUESTION | theDefault));
}

bool Phd::PhdArxSysDlg::getFileNameForRead(LPCTSTR prompt, LPCTSTR def, LPCTSTR ext, CString& fname, bool defIsDirectory /*= true*/, bool allowArbitraryExt /*= false*/)
{
...
bool Phd::PhdArxSysDlg::getFileNameForWrite(LPCTSTR prompt, LPCTSTR def, LPCTSTR ext, CString& fname, bool defIsDirectory /*= true*/, bool allowArbitraryExt /*= false*/)
{
	int flags = 2;          // disable the silly "type it" button
	if (true)
		flags += 1;
...
bool Phd::PhdArxSysDlg::SelFileByDlg(const std::vector<CString>& vecFileType, CString& strPath)
{
...
bool Phd::PhdArxSysDlg::SelDirByDlg(CString& strDirPath)
{
...
bool Phd::PhdArxSysDlg::SaveFileByDlg(CString& strSavePath, const std::vector<CString>& vecFileType, LPCTSTR szFileName /*= NULL*/)
{
...
bool Phd::PhdArxSysDlg::SelFileByDlg(const std::vector<CString>& vecFileType, std::vector<CString>& vecFilePath)
{
	CString strType = _T("所有文件(*.*)|*.*||");
	for (int i = 0; i < vecFileType.size(); i++)
	{
