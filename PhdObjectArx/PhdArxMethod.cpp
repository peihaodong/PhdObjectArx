...
#pragma region 内部函数
void Phd::PhdArxMethod::StringAdded(int nSum, CString& FinalStr, int divisionValue,
...
CTime Phd::PhdArxMethod::GetTime(LPCTSTR szTime)
{
...
Phd::PhdArxMethod::PhdArxMethod()
	:m_apPhdArxInline(std::make_shared<Phd::PhdArxInline>())
{

}

CString Phd::PhdArxMethod::GetAddedString(const CString& str, int nNum, std::vector<CString>& vecStr)
{
...
bool Phd::PhdArxMethod::ClearDir(LPCTSTR szDirPath)
{
	CFileFind finder;
	CString path;
...
bool Phd::PhdArxMethod::DeleteDir(LPCTSTR szDirPath)
{
	CFileFind finder;
	CString path;
	path.Format(_T("%s\\*.*"), szDirPath);
...
bool Phd::PhdArxMethod::CopyDir(LPCTSTR szOldDirPath, LPCTSTR szNewDirPath)
{
...
bool Phd::PhdArxMethod::CopyDirOfFile(LPCTSTR szOldDirPath, LPCTSTR szNewDirPath)
{
	if (!::PathIsDirectory(szOldDirPath))
		return false;
...
bool Phd::PhdArxMethod::DeleteFileOrDir(LPCTSTR szPath)
{
...
bool Phd::PhdArxMethod::CopyFileOrDir(LPCTSTR szFrom, LPCTSTR szTo)
{
// 	SHFILEOPSTRUCT FileOp = { 0 };
...
bool Phd::PhdArxMethod::MoveFileOrDir(LPCTSTR szFrom, LPCTSTR szTo)
{
// 	SHFILEOPSTRUCT FileOp = { 0 };
// 	FileOp.fFlags = FOF_NOCONFIRMATION |   //不出现确认对话框
// 		FOF_NOCONFIRMMKDIR; //需要时直接创建一个文件夹,不需用户确定
...
bool Phd::PhdArxMethod::RenameFileOrDir(LPCTSTR szOldName, LPCTSTR szNewName)
{
// 	SHFILEOPSTRUCT FileOp = { 0 };
// 	FileOp.fFlags = FOF_NOCONFIRMATION |   //不出现确认对话框
// 		FOF_FILESONLY;
...
bool Phd::PhdArxMethod::OpenFile(LPCTSTR szFilePath, LPCTSTR szParame /*= NULL*/)
{
...
bool Phd::PhdArxMethod::LaunchExe(LPCTSTR szFilePath, LPCTSTR szParame /*= NULL*/)
{
	//启动ZWCAD
...
bool Phd::PhdArxMethod::CloseCurOpenedFile(LPCTSTR szFileName)
{
	HWND hWnd = NULL;
...
CString Phd::PhdArxMethod::GetAppDirPath()
{
	//此方法也可以
// 	CString strPath;
...
void Phd::PhdArxMethod::CloseProcess(LPCTSTR sProcessName)
{
	bool bFind = false;
...
int Phd::PhdArxMethod::GetProcessExitReturn(LPCTSTR szFilePath, LPCTSTR szParame /*= NULL*/)
{
...
void Phd::PhdArxMethod::RemoveSameEle(std::vector<double>& vecElement, double dTolerance /*= 0.0001*/)
{
	for (int i = 0; i < vecElement.size() - 1; i++)
	{
		for (int j = i + 1; j < vecElement.size(); j++)
		{
			if (m_apPhdArxInline->IsEqual(vecElement.at(i), vecElement.at(j), dTolerance))
			{
...
AcArray<CString> Phd::PhdArxMethod::GetAllDwgPath(LPCTSTR szFolderPath)
{
...
bool Phd::PhdArxMethod::LaunchCAD(CString strCadPath)
{
	STARTUPINFO si; //一些必备参数设置
	memset(&si, 0, sizeof(STARTUPINFO));
	si.cb = sizeof(STARTUPINFO);
...
CString Phd::PhdArxMethod::GetUniqueName(LPCTSTR szPath)
{
...
bool Phd::PhdArxMethod::FileIsExist(LPCTSTR szFilePath)
{
	//判断文件或文件夹是否存在
...
CString Phd::PhdArxMethod::GetCurSystemTime()
{
...
bool Phd::PhdArxMethod::CompareTime(LPCTSTR szTime1, LPCTSTR szTime2)
{
...
CString Phd::PhdArxMethod::PathToName(LPCTSTR szPath)
{
	CString strPath = szPath;
...
int Phd::PhdArxMethod::GetStringCount(LPCTSTR str, TCHAR s)
{
	CString strTemp = str;
	int nCount = 0;
...
CString Phd::PhdArxMethod::GetAppDataLocalFolder()
{
...
CString Phd::PhdArxMethod::GetAppDataRoamingFolder()
{
	TCHAR szPath[_MAX_PATH];
...
bool Phd::PhdArxMethod::GetHostNameAndIP(CString& strName, CString& strIp)
{
	USES_CONVERSION;
...
CString Phd::PhdArxMethod::EncryptDecode(LPCTSTR szStr, int nNum)
{
...
CString Phd::PhdArxMethod::Encrypt(LPCTSTR szStr, int nNum)
{
	CString strTemp = szStr;
	if (strTemp == _T(""))
		return _T("");
...
CString Phd::PhdArxMethod::Decode(LPCTSTR szStr, int nNum)
{
	CString strTemp = szStr;
	if (strTemp == _T(""))
		return _T("");
...
CString Phd::PhdArxMethod::Decode(TCHAR szS, int nNum)
{
	CString strNew;
	int n = szS;
	int nEncrypt = n - nNum;
...
int Phd::PhdArxMethod::GetRandomInt(int a, int b)
{
...
CTime Phd::PhdArxMethod::GetTimeOfDayLast(const CTime& time, int nDay)
{
...
// void Phd::PhdArxMethod::PurgeDatabase(AcDbDatabase* pDb)
// {
// 	//块
